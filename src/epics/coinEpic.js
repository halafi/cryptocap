...
import {
  FETCH_COIN_LIST,
  FETCH_COIN_EXTRAS,
  fetchCoinListFulfilled,
  fetchCoinExtrasFulfilled,
} from '../actions/index'
...
const fetchCoinListEpic = action$ =>
  action$.ofType(FETCH_COIN_LIST).flatMap(() =>
    coinMarketCapApi
      .getCoinList()
...
const fetchCoinExtrasEpic = action$ =>
  action$.ofType(FETCH_COIN_EXTRAS).flatMap(() =>
    cryptoCompareApi
      .getCoinList()
...
export default combineEpics(fetchCoinListEpic, fetchCoinExtrasEpic)
